...
class OpStudent(models.Model):
    ...
    id_card_number = fields.Char('ID Card Number', size=15)

    # Class selection (3rd to 12th)
...
members teaching this student.""")
    ...
    def _onchange_name(self):
        if not self.middle_name:
            self.name = str(self.first_name or '') + " " + str(
                self.last_name or ''
            )
        else:
            self.name = str(self.first_name or '') + " " + str(
    ...
    @api.constrains('id_card_number', 'gr_no')
    def _check_numeric_fields(self):
        for rec in self:
            if rec.id_card_number:
                if not rec.id_card_number.isdigit():
                    raise ValidationError(_("ID Card Number must contain only digits."))
                if not (6 <= len(rec.id_card_number) <= 20):
                    raise ValidationError(_("ID Card Number must be between 6 and 20 digits."))
            if rec.gr_no:
                if not rec.gr_no.isdigit():
                    raise ValidationError(_("Registration Number must contain only digits."))
